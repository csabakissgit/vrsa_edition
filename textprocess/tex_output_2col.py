...
def flushOutApparatus(collectedApp):
            if collectedApp != '':
                print('\\varr{%' + collectedApp + '}')
            return 0

def tex_output_2col(filename):
    chapter = 0
    ...
    for line in openfile:
        ...
        if '<START/>' in line:
            onflag = True
        if '<STOP/>' in line:
            flushOutApparatus(collectedApp)
            collectedApp = ''
            onflag = False
        ...
        if ('<LACUNA>' in line or lacunaflag == True) and onflag == True:
            ...
            if '</LACUNA>' in line:
                lacunaflag = False
            v01 = re.sub('{ }', " ", line)
            v01 = re.sub('<LACUNA>', '    \\\\lacuna{\\\\textit{', v01[:-1])
            v01 = re.sub('</LACUNA>', '}}', v01)
            v01 = re.sub('{ }', " ", v01)
            v01 = re.sub('\\Ł', '{\\\\normalfont ', v01)
            v01 = re.sub('\\$', '}', v01)
            v01 = re.sub('\|\|', '{\\\\thinspace\\\\ketdanda}', v01)
