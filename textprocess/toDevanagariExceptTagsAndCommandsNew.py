...
def main(line):
    ...
    def preprocessing_new(inputline,vowels,consonants,devnagFlag):
            # print(inputword)
            # to make double letter one
            # trick : "|" becomes " |" to handle final virama
            preprocessingChars = [('ai', 'đ'), ('au', 'ő'), ('kh', 'K'), ('gh', 'G'), ('ṭh', 'Ṭ'), ('ḍh', 'Ḍ'), ('th', 'T'), ('dh', 'D'),('ph', 'P'), ('bh', 'B'), ('ch', 'C'), ('jh', 'J'), ('\|', ' |'), ('\| \|', '||'), (",", " ,")]
            ...
            counter = 0
            preprocessedLine = ''
            while counter < len(inputline)-1:
                newchar = inputline[counter]
                if newchar == 'Ł':
                        devnagFlag = False
                if newchar == '$':
                        devnagFlag = True
                if devnagFlag == True:                                           
                        for p in preprocessingChars:
                                if inputline[counter]+inputline[counter+1] == p[0]:
                                        newchar = p[1]
                                        counter += 1
                preprocessedLine = preprocessedLine + newchar
                newchar = ''
                counter += 1
            print('\n!!!',devnagFlag, preprocessedLine)
            # spaces, sandhi
            # list of characters separated
            s = list(preprocessedLine)
            i = 0
            # C + V should be written as conjunct 
            while i < len(s)-2:
                    if s[i] in consonants and s[i+1] == '€' and s[i+2] in vowels:
