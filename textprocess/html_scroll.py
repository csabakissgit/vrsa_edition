...
from textprocess import bib_substitutions
            ...
            if onflag is True:
                ...
                if PARAL_OPEN_TAG in line or paralflag is True:
                    ...
                    if PARAL_CLOSING_TAG in line:
                        paralflag = False
                    outputline = re.sub("{ }", " ", line)
                    outputline = re.sub("Ł", " ", outputline)
                    outputline = re.sub("\$", " ", outputline)
                    outputline = re.sub(" *\\\\vo", str(vsnum + uvacaflag), outputline)
                    outputline = re.sub(
                        PARAL_OPEN_TAG + " *\\\\v",
                ...
                if "<TR>" in line or trflag is True:
                    ...
                    # dealing with TeX-oriented indexing etc.
                    outputline = re.sub("-##", "<idx>", outputline)
                    outputline = re.sub("##-", "</idx>", outputline)
                    outputline = re.sub("-##?", "", outputline)
                    outputline = re.sub("##?-", "", outputline)
                    outputline = re.sub("\\\\enx", "", outputline)
                    outputline = re.sub("\\\\myidx", "!!!", outputline)
                    outputline = re.sub("\\\\mysktindex", "!!!", outputline)
                    outputline = re.sub("\\\\index", "!!!", outputline)

                    outputline = re.sub("\\\\linebreak", "", outputline)
                    outputline = re.sub("---", "—", outputline)
                ...
                if NOTE_OPEN_TAG in line or noteflag is True:
                    ...
                    outputline = re.sub("\\\\mysktindex", "!!!", outputline)
                    outputline = re.sub("\\\\index", "!!!", outputline)
                    outputline = re.sub("\\\\citeauthor", "", outputline)
                    ...
                    if NOTE_CLOSING_TAG in line:
                        ...
                        collected_notes = bib_substitutions.main(collected_notes)
                    else:
                        collected_notes = collected_notes + outputline
