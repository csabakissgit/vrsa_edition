...
def turnVsnumIntoDevnagNums(chapter, vsnum):
        ...
        return devnagChapter + ':\\\\hspace{.11em}' + devnagVsnum
        ## to have only the verse number printed for each verse
        #return devnagVsnum

def tex_dn_xelatex(filename, LaTeXDn):
    if LaTeXDn:
        englishfont = '\\\\rm'
        devanagarifont = '\\\\dn'
        ...
        englishfont = '\\\\englishfont'
        devanagarifont = '\\\\devanagarifont'
        devanagarifontsmall = '\\\\devanagarifontsmall'
        devanagarifontbold = '\\\\devanagarifontbold'
        devanagarifontvar = '\\\\devanagarifontvar'
        danda = '।'
        twodandas = '॥'
    romanflag = False
    chapter = 0
...
#            print("\n\\nemsloka% ")
        ...
        if ('<APP>' in line or appflag == True) and onflag == True:
            ...
            v01 = re.sub(",", ",\\\\hskip.5em plus .9em", v01)
            # if the ; after the lemma's witnesses should be a , instead
            v01 = re.sub(";", ",\\\\hskip.2em plus .9em", v01)
