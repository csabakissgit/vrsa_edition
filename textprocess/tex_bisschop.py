...
#            print("\n\\nemsloka% ")
        ...
        if '<NEWCHAPTER/>' in line and onflag == True:
            if firstchapter_flag == False:
                # not the first chapter to process:
                print("\\bekveg\\szamveg\\vfill\\phpspagebreak\\szam\n\\bek\\fejno=" + str(chapter) + "\n")
                # augment chapter number
                v01 = "\nBACKSLASHjumpBACKSLASHnewchapter"
            else:
                # the first chapter to process:
                chapter += 1
                v01 = "BACKSLASHujfejBACKSLASHszam\nBACKSLASHbek\\fejno=" + str(chapter) + "\n"
        ...
        if ('<TEXT>' in line or textflag == True) and onflag == True:
            line = re.sub("{-}", "", line)
            line = re.sub('<crux>', '{\\\\englishfont †}', line)
            line = re.sub('</crux>', '{\\\\englishfont †}', line)
            ...
            if proseflag == False:
                v01 = re.sub('<TEXT> ?', '\n{\\\\devanagarifontbold ', outputline[:-1])
            if proseflag == True:
        ...
        if ('<APP>' in line or appflag == True) and onflag == True:
            ...
            line = re.sub("°", "॰", line)
            appflag = True
            v01 = re.sub('<APP> ?', '    \\\\var{{\\\\devanagarifont ' + appstartforverse, line[:-1])
            if '</APP>' in line:
                appflag = False
            v01 = re.sub(' ?</APP>', '}}%', v01)
            v01 = re.sub('{ }', " ", v01)
            ...
            #v01 = re.sub(',', '\ ', v01)
            #v01 = re.sub(';', '\ ', v01)
            #v01 = re.sub('\\\\va ', '{\\\\englishfont ' + str(vsnum) + 'a }', v01)
            #v01 = re.sub('\\\\vb ', '{\\\\englishfont ' + str(vsnum) + 'b }', v01)
            #v01 = re.sub('\\\\vc ', '{\\\\englishfont ' + str(vsnum) + 'c }', v01)
            #v01 = re.sub('\\\\vd ', '{\\\\englishfont ' + str(vsnum) + 'd }', v01)
...
#                   v01 = re.sub('{ }', " ", v01)
        ...
        if '<CHAPTER>' in line and onflag == True:
            ...
            v01 = re.sub('{ }', " ", v01)
            print('\\fancyhead[CO]{{\\footnotesize\\devanagarifont ' + headertitle + '}}\n\\fancyhead[CE]{{\\footnotesize\\devanagarifont ' + header + '}}\n\\fancyhead[LE]{}\n\\fancyhead[RE]{}\n\\fancyhead[LO]{}\n\\fancyhead[RO]{}\n\\szam\\bek\n')
            print(v01, end="")
