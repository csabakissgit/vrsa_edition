...
from textprocess import tex_output_2col
from textprocess import tex_dn_output
from textprocess import tex_dn_xelatex
from textprocess import tex_roman_xelatex
from textprocess import tex_bisschop
...
from textprocess import trtexstandalone
from textprocess import change_sigla 
from textprocess import html_scroll 
from textprocess import index 
from textprocess import folioinfo
from textprocess import tr_and_notes
from textprocess import rtf_output
...
if len(sys.argv) == 2 and sys.argv[1] == '-help':
    ...
    quit()
#elif len(sys.argv) != 3 and (sys.argv[1] != '-appcheck' or sys.argv[1] != '-foliobreaks'):
...
elif sys.argv[1] == '-texdnlatex': 
    tex_dn_xelatex.tex_dn_xelatex(filename, True)
elif sys.argv[1] == '-texdnxelatex':
    tex_dn_xelatex.tex_dn_xelatex(filename, False)
elif sys.argv[1] == '-texrmxelatex':
    tex_roman_xelatex.tex_roman_xelatex(filename, False)
elif sys.argv[1] == '-texbisschop':
    tex_bisschop.main(filename)
elif sys.argv[1] == '-texrm': 
    tex_output.tex_output(filename)
elif sys.argv[1] == '-texrm2col': 
    tex_output_2col.tex_output_2col(filename)
elif sys.argv[1] == '-txt': 
...
elif sys.argv[1] == '-scroll' or sys.argv[1] == '-html': 
    html_scroll.html_scroll(filename)
...
elif sys.argv[1] == '-trtexstandalone': 
    trtexstandalone.trtex(filename)
elif sys.argv[1] == '-index': 
    index.index(filename)
elif sys.argv[1] == '-folioinfo': 
    folioinfo.main(filename)
elif sys.argv[1] == '-trnotes': 
    tr_and_notes.tr_and_notes(filename)
elif sys.argv[1] == '-rtf': 
    rtf_output.rtf_output(filename)
# mainly for vim and command line:
elif sys.argv[1] == '-line2dn': 
    line_to_dn.line_to_dn(sys.argv[2])
else:
    print("Usage: textprocess.py [-help|-txt|-txtdn|-texrm|-texdn|-velth|-html|-tr|-sktandtr|-trtex|-xml1|-xml2|-xml3|-xmldn1|xmldn2|-appcheck|-index]")
