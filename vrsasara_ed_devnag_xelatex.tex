% !TEX TS-program = xelatex
% !TEX encoding = UTF-8

\documentclass[a4paper, 12pt]{book}

\usepackage{fontspec} % Font selection for XeLaTeX; see fontspec.pdf for documentation
\defaultfontfeatures{Mapping=tex-text} % to support TeX conventions like ``---''
\usepackage{xunicode} % Unicode support for LaTeX character names (accents, European chars, etc)

%\defaultfontfeatures{Mapping=tex-text} % ditto

% The polyglossia package lets us easily use several languages.
\usepackage{polyglossia}

% Define the used fonts. Replace Nakula and XITS by your font choice.

%\newfontfamily\devanagarifont[Script=Devanagari]{Pedantic Indic}
%\newfontfamily\devanagarifont[Script=Devanagari]{Sahadeva}
%\newfontfamily\devanagarifont[Scale=1.5,Script=Devanagari]{AdishilaDev}
\newfontfamily\devanagarifont[Scale=0.9,Script=Devanagari]{Noto Serif Devanagari}
\newfontfamily\englishfont[Scale=1,Script=Latin]{EB Garamond}

%\setmainlanguage{english}
%\setotherlanguages{sanskrit}

\setmainfont{AdishilaDev} 
\usepackage{xltxtra} % Extra customizations for XeLaTeX
%\setsansfont{Deja Vu Sans}
%\setmonofont{Deja Vu Mono}
% other LaTeX packages.....
\usepackage[total={11.7cm,17cm},
top=4cm, left=5cm, headsep=1.1cm, footskip=6cm, footnotesep=1cm]{geometry}
\renewcommand{\baselinestretch}{.95}
\renewcommand{\baselinestretch}{.9}





\input{/home/csaba/indology/dharma_project/vrsa_edition/vrsasara_macros_devnag_xelatex.tex}

\begin{document}
% ADJUST PAGE NUMBER for EDITION ! 
\setcounter{page}{500}

\input{/home/csaba/indology/dharma_project/vrsa_edition/vrsasara_input_devnag_xelatex.tex}

\bekveg\szamveg
\end{document}

